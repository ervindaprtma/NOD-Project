...
function SankeyView({ data, loading, error }: { data: SankeyResponse | undefined; loading: boolean; error: boolean }) {
  ...
  if (loading) return <Card><div className="h-[400px] bg-muted rounded animate-pulse flex items-center justify-center"><p className="text-sm text-muted-foreground">Loading sankey data...</p></div></Card>;
  if (error) return <Card><ErrorText /></Card>;
  if (!data?.nodes?.length) return <Card><EmptyState message="No sankey data available for this time range" /></Card>;
  return (
    <Card>
