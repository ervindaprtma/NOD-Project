...
export default function TrafficInboundPage() {
  ...
  const [siteName, setSiteName] = useState("Site_FGT-DRC");
  ...
  const summaryKey = token
    ? `/api/v1/traffic-inbound/summary?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}`
    : null;
  const chartKey = token
    ? `/api/v1/traffic-inbound/chart?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}&bucket_seconds=${bucketSeconds}`
    : null;
  const tableKey = token
    ? `/api/v1/traffic-inbound/table?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}`
    : null;
  ...
  // ── Sankey data fetches (Upload + Download) ──
  const sankeyUploadKey = token
    ? `/api/v1/traffic-inbound/sankey?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}&direction=upload`
    : null;
  const sankeyDownloadKey = token
    ? `/api/v1/traffic-inbound/sankey?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}&direction=download`
    : null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground/70 mt-0.5 font-medium">
            VIP Inbound — flow.traffic.path = inbound-vip
          </p>
        </div>
      ...
      {hasError && (
        <div className="p-4 rounded-lg bg-destructive/10 text-destructive text-sm">
          Failed to load traffic inbound data.{" "}
          <button
          ...
          <TabPanel>
            ...
            {!summaryLoading && !chartLoading && !tableLoading && !hasError && !summary && !chart && !table && (
              <div className="p-4 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 text-sm">
                No inbound traffic data returned for {siteName.replace(/_/g, " ")}.
                The device may be unreachable or has not reported metrics yet.
              </div>
            )}

            {/* ═══ SECTION 1 — 9 Summary Widgets ═══ */}
            ...
            {/* ═══ SECTION 3 — Stacked Bar Chart (Service-based) ═══ */}
            <Card>
...
const LEVEL_COLORS: Record<number, string> = {
  0: "#3b82f6", // AS Org Source — blue
  1: "#22c55e", // Ingress — green
  2: "#f97316", // Service — orange
  3: "#a855f7", // Egress — purple
};
...
function SankeyView({ data, loading, error }: {
  ...
  if (loading) {
    return (
      <Card>
        <Title className="mb-3">Sankey Diagram</Title>
        <div className="h-[500px] bg-muted rounded animate-pulse flex items-center justify-center">
  ...
  if (!data || !data.nodes || data.nodes.length === 0) {
    return (
      <Card>
        <Title className="mb-3">Sankey Diagram</Title>
        <EmptyState message="No sankey data available for this time range" />
      </Card>
    );
  }
              ...
              <div key={i} className="flex items-center gap-2 text-xs">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <span className={`truncate ${item.mono ? "font-mono text-[11px]" : ""}`}>
                      {item.name}
                    </span>
...
function FlowRecordsTable({
  ...
  records: TrafficInboundTableRecord[];
}) {
  return (
    <div className="overflow-x-auto">
...
// ── Stacked Bar Chart (SVG — dynamic per-service colors) ────────
...
function StackedBarChart({
  ...
  const [hoveredBar, setHoveredBar] = useState<{
    barIndex: number;
    serviceBreakdown: { service: string; mbps: number; color: string }[];
    x: number;
  } | null>(null);

  if (!data.length || !serviceNames.length) {
    return <EmptyState message="No service throughput data" />;
  }
  ...
  serviceNames.forEach((svc, i) => {
    colorMap[svc] = serviceColor(i, serviceNames.length);
  });
  ...
  function xScale(i: number) {
    return pad.left + i * (plotW / data.length);
  }

  function yScale(v: number) {
    return pad.top + plotH - (v / maxTotal) * plotH;
  }
  ...
  return (
    <div className="relative">
      ...
      <div className="flex flex-wrap gap-x-4 gap-y-1 mb-3">
        {serviceNames.slice(0, 25).map((svc) => (
          <div key={svc} className="flex items-center gap-1.5 text-[11px]">
            <span
              className="w-3 h-3 rounded-sm shrink-0"
              style={{ backgroundColor: colorMap[svc] }}
            />
        ...
        {serviceNames.length > 25 && (
          <span className="text-[11px] text-muted-foreground">
            +{serviceNames.length - 25} more
          </span>
        )}
      </div>
              ...
              {serviceNames.map((svc) => {
                ...
                const el = (
                  <rect
                    ...
                    onMouseEnter={(e) => {
                      const breakdown = serviceNames
                        .filter((s) => (Number(row[s]) || 0) > 0)
                        .map((s) => ({
                          service: s,
                          mbps: Number(row[s]) || 0,
                          color: colorMap[s],
      ...
      {hoveredBar && (
        <div
          className="fixed z-50 bg-card border rounded-lg shadow-lg p-3 text-xs pointer-events-none"
          style={{
            left: Math.min(hoveredBar.x, window.innerWidth - 260),
            top: 60,
            maxWidth: 250,
          }}
        >
          <p className="font-semibold mb-2 text-muted-foreground">
            {formatTs(data[hoveredBar.barIndex])}
          </p>
          ...
          <div className="border-t mt-2 pt-1.5 flex justify-between font-semibold">
            ...
            <span className="font-mono">
              {hoveredBar.serviceBreakdown
                .reduce((s, a) => s + a.mbps, 0)
                .toFixed(2)}{" "}
              Mbps
            </span>
          </div>
