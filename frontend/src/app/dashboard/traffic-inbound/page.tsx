...
function countActiveFilters(f: FilterState): number {
  let n = 0;
  if (f.application) n++; if (f.client_ip) n++; if (f.server_ip) n++;
  if (f.protocol) n++; if (f.dst_port) n++;
  return n;
}
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder="..."
        className={cn("px-2 py-1.5 text-xs rounded border border-border/60 dark:border-border/40 bg-background focus:outline-none focus:ring-1 focus:ring-primary/30", mono && "font-mono text-[11px]")} />
    </div>
  );
}
