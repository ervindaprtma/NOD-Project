...
const SITES = ["Site_FGT-DC", "Site_FGT-DRC"];
const SITE_LABELS: Record<string, string> = { "Site_FGT-DC": "DC", "Site_FGT-DRC": "DRC" };
...
export default function TrafficInboundPage() {
  ...
  const summaryKey = token
    ? `/api/v1/traffic-inbound/summary?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}${filterQS}` : null;
  const chartKey = token
    ? `/api/v1/traffic-inbound/chart?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}&bucket_seconds=${bucketSeconds}${filterQS}` : null;
  const tableKey = token
    ? `/api/v1/traffic-inbound/table?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}${filterQS}` : null;
  const sankeyUploadKey = token
    ? `/api/v1/traffic-inbound/sankey?site_name=${siteName}&gte_ms=${currentGteMs}&lte_ms=${currentLteMs}&direction=upload${filterQS}` : null;
  ...
  const { data: summaryEnv, error: summaryError, isLoading: summaryLoading } = useSWR<{ success: boolean; data: TrafficInboundSummary; meta: ResponseMeta | null }>(summaryKey, swrFetcherLong, { refreshInterval: 0 });
  const { data: chartEnv, error: chartError, isLoading: chartLoading } = useSWR<{ success: boolean; data: TrafficInboundChartData; meta: ResponseMeta | null }>(chartKey, swrFetcherLong, { refreshInterval: 0 });
  const { data: tableEnv, error: tableError, isLoading: tableLoading } = useSWR<{ success: boolean; data: TrafficInboundTableData; meta: ResponseMeta | null }>(tableKey, swrFetcherLong, { refreshInterval: 0 });
  ...
  function handlePreset(seconds: number, label: string) {
    const now = Date.now();
    setGteMs(now - seconds * 1000); setLteMs(now);
    setActivePresetSeconds(seconds); setSelectedPreset(label);
    setCustomRangeLabel(null); setShowCustomPicker(false);
    setRefreshInterval((prev) => prev === 0 ? prevIntervalRef.current : prev);
  }

  function handleCustomApply(range: CustomTimeRange) {
    setGteMs(range.gte_ms); setLteMs(range.lte_ms);
    setActivePresetSeconds(0); setSelectedPreset("custom");
    prevIntervalRef.current = refreshInterval > 0 ? refreshInterval : DEFAULT_REFRESH_MS;
    setRefreshInterval(0); setShowCustomPicker(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <h1 className="text-2xl font-bold tracking-tight text-foreground">Traffic Inbound</h1>
        <div className="flex items-center gap-3 flex-wrap">
          <select value={siteName} onChange={(e) => setSiteName(e.target.value)}
            className="px-3 py-1.5 rounded-md border border-border/60 dark:border-border/40 bg-background text-sm shadow-sm dark:shadow-none dark:ring-1 dark:ring-white/20 focus:outline-none focus:ring-2 focus:ring-primary/20">
      ...
      {hasError && (
        <div className="p-4 rounded-lg bg-destructive/10 text-destructive text-sm">
          Failed to load traffic data. <button onClick={() => window.location.reload()} className="underline">Retry</button>
        </div>
      )}
        ...
        {showFilters && (
          <div className="px-4 pb-4 pt-1 border-t border-muted/40">
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3">
              <TagFilterField label="Application" values={draftFilters.application} onChange={(v) => setDraftFilters({ ...draftFilters, application: v })} />
        ...
        <TabsContent value="overview">
            {!summaryLoading && !chartLoading && !tableLoading && !hasError && !summary && (
              <div className="p-4 rounded-lg bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300 text-sm">
                No traffic inbound data returned for {SITE_LABELS[siteName] || siteName}.
              </div>
            )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
              ...
              <RankedCard title="Top AS Client" loading={summaryLoading} error={!!summaryError}
                items={(summary?.top_src_as_org || []).slice(0, 10).map(o => ({ name: o.org_name, value: o.total_bytes }))} color="emerald" wide />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
              ...
              <RankedCard title="Top Countries Client" loading={summaryLoading} error={!!summaryError}
            ...
            <div className="space-y-4 mb-6">
              <div className="bg-card border border-border/60 dark:border-border/40 rounded-lg shadow-sm dark:shadow-none dark:ring-1 dark:ring-white/20 p-6">
                ...
                {chartLoading ? <SkeletonChart /> : chartError ? <ErrorText />
                  : throughputTimeline.length > 0 ? (
                    <AreaChart data={throughputTimeline} categories={["mbps"]} index="timestamp"
                      valueFormatter={(v: number) => v >= 100 ? `${v.toFixed(0)} Mbps` : `${v.toFixed(2)} Mbps`} colors={["#3b82f6"]} showLegend={false} showGridLines={true} showXAxis={true} showYAxis={true}
              ...
              <div className="bg-card border border-border/60 dark:border-border/40 rounded-lg shadow-sm dark:shadow-none dark:ring-1 dark:ring-white/20 p-6">
                <ChartHeader title={`Service Throughput — ${bucketSeconds}s Buckets (up + down)`} isZoomed={isZoomed} onReset={resetZoom} />
            ...
            <div className="bg-card border border-border/60 dark:border-border/40 rounded-lg shadow-sm dark:shadow-none dark:ring-1 dark:ring-white/20 p-6">
              ...
              ) : tableError ? <ErrorText />
                : table?.records?.length ? <FlowRecordsTable records={table.records.slice(0, 25)} />
...
// ── Sankey (reused from Traffic Internet, same component) ─────────

const LEVEL_COLORS: Record<number, string> = { 0: "#3b82f6", 1: "#22c55e", 2: "#f97316", 3: "#a855f7" };
const LEVEL_LABELS: Record<number, string> = { 0: "AS Org", 1: "Ingress", 2: "Service", 3: "Zone" };
...
function SankeyView({ data, loading, error }: { data: SankeyResponse | undefined; loading: boolean; error: boolean }) {
  ...
  useEffect(() => {
    ...
    for (const link of graph.links) {
      ...
      p.appendChild(ce("title", {})); p.firstChild!.textContent = `${src.label} → ${tgt.label}: ${link.value.toFixed(1)} MB`;
      g.appendChild(p);
      if (((link as any).width ?? 0) > 3 && link.value > 0.5) {
        const mx = ((src.x1 ?? 0) + (tgt.x0 ?? 0)) / 2, my = ((link as any).y0 + (link as any).y1) / 2;
        const t = ce("text", { x: mx, y: my, dy: "0.35em", "text-anchor": "middle", "font-size": ((link as any).width ?? 0) > 8 ? "10" : "8", fill: "currentColor", class: "fill-muted-foreground" });
        t.textContent = link.value >= 1000 ? `${(link.value / 1000).toFixed(1)} GB` : `${link.value.toFixed(1)} MB`;
        g.appendChild(t);
      }
      svgEl.appendChild(g);
    }
    for (const node of graph.nodes) {
      ...
      r.appendChild(ce("title", {})); r.firstChild!.textContent = `${node.label}\n${LEVEL_LABELS[node.level] || "L" + node.level}\n${(node.value ?? 0).toFixed(1)} MB`;
      g.appendChild(r);
      const mx = ((node.x0 ?? 0) + (node.x1 ?? 0)) / 2, isL = mx < W / 2;
      const t = ce("text", { x: isL ? (node.x1 ?? 0) + 4 : (node.x0 ?? 0) - 4, y: ((node.y0 ?? 0) + (node.y1 ?? 0)) / 2, dy: "0.35em", "text-anchor": isL ? "start" : "end", "font-size": "10", fill: "currentColor", class: "fill-foreground" });
      t.textContent = (node.label || "").length > 24 ? (node.label || "").slice(0, 22) + "\u2026" : node.label || "";
      g.appendChild(t);
      svgEl.appendChild(g);
    }
  }, [data]);

  const CARD_CLS = "bg-card border border-border/60 dark:border-border/40 rounded-lg shadow-sm dark:shadow-none dark:ring-1 dark:ring-white/20 p-6";
  if (loading) return <div className={CARD_CLS}><div className="h-[400px] bg-muted rounded animate-pulse flex items-center justify-center"><p className="text-sm text-muted-foreground">Loading sankey data...</p></div></div>;
  if (error) return <div className={CARD_CLS}><ErrorText /></div>;
  if (!data?.nodes?.length) return <div className={CARD_CLS}><EmptyState message="No sankey data available for this time range" /></div>;
  return (
    <div className={CARD_CLS}>
      <div className="overflow-x-auto"><svg ref={svgRef} viewBox="0 0 800 400" className="w-full" style={{ minWidth: 500 }} /></div>
    </div>
  );
}
...
function RankedCard({ title, loading, error, items, color, wide }: { title: string; loading: boolean; error: boolean; items: RankedItem[]; color: string; wide?: boolean }) {
  ...
  return (
    <div className={CARD_CLS}>
      ...
      {loading ? <SkeletonBars count={5} /> : error ? <ErrorText />
        : items.length > 0 ? (
                ...
                <div key={i} className="flex items-center gap-2 text-xs">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-0.5">
                      <span className={`truncate ${item.mono ? "font-mono text-[11px]" : ""}`} title={item.name}>{item.name}</span>
                      <span className="ml-2 text-muted-foreground shrink-0">{formatBytes(item.value)}</span>
                    </div>
                    <div className={`h-1.5 rounded-full ${c.bg}`}><div className={`h-full rounded-full ${c.bar}`} style={{ width: `${pct}%`, opacity: 1 - i * 0.06 }} /></div>
                  </div>
                </div>
              );
            })}
        ...
        <tbody>
          {records.map((r, i) => (
            <tr key={i} className="border-b border-muted/50 last:border-0 hover:bg-muted/30 transition-colors">
              <td className="py-1.5 pr-3 font-mono text-[11px]">{r.client_ip}</td>
              <td className="py-1.5 pr-3 font-mono text-[11px]">{r.server_ip}</td>
              <td className="py-1.5 pr-3 truncate max-w-[150px]">{r.service}</td>
...
// ── Stacked Bar Chart (with hover tooltips, from traffic-inbound) ─

function serviceColor(index: number, total: number): string {
  const hue = (index * 360) / total, sat = 70 + (index % 3) * 10, lit = 45 + (index % 4) * 8;
...
function StackedBarChart({ data, serviceNames, onRangeSelect, bucketMs = 60_000 }: { data: Record<string, any>[]; serviceNames: string[]; onRangeSelect?: (gteMs: number, lteMs: number) => void; bucketMs?: number }) {
  ...
  let maxTotal = 0;
  for (const row of data) { let sum = 0; for (const svc of serviceNames) sum += Number(row[svc]) || 0; if (sum > maxTotal) maxTotal = sum; }
  if (maxTotal === 0) maxTotal = 1;
  const barWidth = Math.max(2, plotW / data.length - barGap);
  const yTickValues = Array.from({ length: 6 }, (_, i) => (maxTotal / 5) * i);
  const colorMap: Record<string, string> = {}; serviceNames.forEach((svc, i) => { colorMap[svc] = serviceColor(i, serviceNames.length); });
  const xScale = (i: number) => pad.left + i * (plotW / data.length);
  const yScale = (v: number) => pad.top + plotH - (v / maxTotal) * plotH;
    ...
    <div className="relative">
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" style={{ maxHeight: 420, cursor: onRangeSelect ? "crosshair" : undefined, touchAction: "none", userSelect: "none" }} {...handlers}>
        ...
        {data.map((row, i) => {
          ...
          return <g key={i}>{serviceNames.map((svc) => {
            const mbps = Number(row[svc]) || 0;
            if (mbps <= 0) return null;
            const barH = (mbps / maxTotal) * plotH, actualY = yOff - barH;
            const el = <rect key={svc} x={x} y={actualY} width={Math.max(1, barWidth - barGap)} height={Math.max(1, barH)} fill={colorMap[svc]} className="cursor-pointer transition-opacity hover:opacity-80"
              onMouseEnter={(e) => {
                const bd = serviceNames.filter((s) => (Number(row[s]) || 0) > 0).map((s) => ({ svc: s, mbps: Number(row[s]) || 0, color: colorMap[s] })).sort((a, b) => b.mbps - a.mbps);
                const rect = (e.target as SVGRectElement).getBoundingClientRect();
                setHoveredBar({ barIndex: i, sBreakdown: bd, x: rect.left + rect.width / 2 });
              }}
      ...
      {hoveredBar && (
        <div className="fixed z-50 bg-card border rounded-lg shadow-lg p-3 text-xs pointer-events-none" style={{ left: Math.min(hoveredBar.x, window.innerWidth - 260), top: 60, maxWidth: 250 }}>
          ...
          <div className="border-t mt-2 pt-1.5 flex justify-between font-semibold"><span>Total</span><span className="font-mono">{hoveredBar.sBreakdown.reduce((s, a) => s + a.mbps, 0).toFixed(2)} Mbps</span></div>
        </div>
      )}
    </div>
  );
