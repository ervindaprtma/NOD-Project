...
class ReportJob(Base):
    ...
    sites: list[str]|None    # JSONB — List of site names to include
    sections: list[str]|None # JSONB — List of sections; None = all
```
...
### 5.3 Overview Dashboard — `app/api/overview.py` (267 lines)
...
**Dynamic Interval Selection (`_compute_interval()`):**
Automatically selects `date_histogram` interval based on time range size:
- ≤ 2 hours → `60s`
- ≤ 12 hours → `5m`
- > 12 hours → `15m`

This interval is passed to `iface_qb.interface_stats_timeline()` for WAN bandwidth queries, ensuring appropriate granularity for any time range.
...
| — | `iface_qb.interface_stats_timeline(interval=_compute_interval(...))` | WAN bandwidth per site (3 sites, dynamic interval) |
| — | `ti_qb.flow_summary()` | Inbound VIP top services |
| — | DB query | Active alert count |

**Mbps Calculation:** Uses `interval_seconds` returned by the query builder (derived from the dynamic interval) instead of a hardcoded 60-second divisor. Formula: `Mbps = (max_current - max_prev) × 8 / interval_seconds / 1,000,000`
...
### 5.10 Interface Stats — `app/api/interface_stats.py` (202 lines)
...
Computes `date_histogram` interval based on time range size (same thresholds as overview.py):
- ≤ 2 hours → `60s` (interval_seconds=60)
- ≤ 12 hours → `5m` (interval_seconds=300)
- > 12 hours → `15m` (interval_seconds=900)

**Flow:**
1. Validate site against `SITE_SOURCE_MAP`
2. Single OpenSearch query for all interfaces (Q-07 compliance)
3. Compute throughput deltas from cumulative counters: `Mbps = (current - previous) × 8 / interval_seconds / 1,000,000` (interval_seconds is dynamic based on time range)
...
### 5.15 Reports — `app/api/reports.py`
...
| ID | Title | Sections |
|---|---|---|
| R-01 | Traffic Internet | traffic_overview (top apps, throughput, AS, countries, protocol, per-site) |
| R-02 | Resource Usage | resource_usage (device status, CPU/memory/session timelines) |
| R-03 | VPN Users | vpn_users (SSL VPN, IPsec VPN) |
| R-04 | SD-WAN SLA | sdwan_sla (latency, jitter, packet loss, link status) |
| R-05 | Traffic Inbound | traffic_inbound (top services, client AS, countries, egress) |
| R-06 | Traffic Internal | traffic_internal (top services, clients, servers) |
| R-07 | Executive Summary | All sections, 1-page KPI layout |
| R-08 | All-in-One | All sections combined |

**Multi-Site Logic:**
- Sites are resolved via `IP_TO_SITE` mapping (IP → canonical site name)
- Each site queries its own OpenSearch cluster (DC → DC cluster, DRC/Office → DRC cluster)
- Results are aggregated per-site for summary tables
- Charts combine data from all selected sites
...
### 6.8 Interface Stats Query Builder — `app/opensearch/interface_stats.py` (203 lines)
...
| Function | Purpose |
|----------|---------|
| `interface_stats_timeline(gte_ms, lte_ms, site_name, interval="60s")` | Per-interface throughput timeline with configurable interval |

**Helpers:**
- `_parse_interval_seconds(interval: str) -> int` — Converts OpenSearch interval string (e.g., `"60s"`, `"5m"`, `"15m"`) to integer seconds for Mbps calculation.

**Return Value:** Returns `{"aggregations": ..., "interval_seconds": int}` — includes the parsed interval in seconds so callers can use it in throughput calculations without redundant parsing.

**Aggregations:** `terms` on `ifIndex` → `date_histogram` (using `fixed_interval` from `interval` param) → `max` on `inOctets`/`outOctets`/`speed`/`ifOperStatus`
...
### 7.2 Report Generator — `app/services/report_generator.py`
...
| ID | Type | Sections |
|----|------|----------|
| R-01 | Traffic Internet | Top apps, throughput, AS, countries, protocol, per-site |
...
**Section Mapping (Frontend → Backend):**

The `FRONTEND_TO_BACKEND_SECTION` dict maps frontend sub-section IDs to backend section keys:
- `R-01`: top_apps, throughput, top_as, top_countries, protocol_dist, per_site → `traffic_overview`
- `R-02`: device_status, cpu_timeline, memory_timeline, session_timeline → `resource_usage`
- `R-03`: ssl_vpn, ipsec_vpn → `vpn_users`
- `R-04`: latency, jitter, packet_loss, link_status → `sdwan_sla`
- `R-05`: top_services, top_client_as, top_countries, egress → `traffic_inbound`
- `R-06`: top_services, top_clients, top_servers → `traffic_internal`

**Multi-Site Logic:**
- Sites resolved via `IP_TO_SITE` mapping (IP → canonical site name)
- `sites_to_query` filters based on selected sites
- DC queries DC cluster, DRC/Office queries DRC cluster
- Results aggregated per-site for summary tables
- Charts combine data from all selected sites
...
**Template:** External Jinja2 template at `reports/templates/report_base.html`. Follows NOD design system with WeasyPrint-compatible CSS (@page A4, header/footer, page breaks). Loaded via `FileSystemLoader`.

**DOCX Generator:** `scripts/generate_docx.py`. Programmatic Word document generation matching the HTML/PDF visual design.

**Chart Integration:** Uses `chart_renderer.py` to embed Matplotlib PNG charts as base64 data URIs. Charts include:
- **R-01:** Throughput timeline (stacked area chart per application)
- **R-02:** CPU timeline, Memory timeline, Active Sessions timeline (line charts per device)

**Report Expiry:** Reports expire after 1 hour (`REPORT_TTL_HOURS = 1`).
...
| API Routes | 17 | 2,926 |
| Schemas | 11 | 1,045 |
| OpenSearch Query Builders | 10 | 3,468 |
| Services | 7 | 2,138 |
