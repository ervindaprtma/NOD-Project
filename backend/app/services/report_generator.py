...
async def build_report_context(
    ...
    if report_type in ("R-02", "R-07", "R-08") and (not sections or "resource_usage" in sections or report_type in ("R-07", "R-08")):
        ...
        # Site mapping: DC=HA members, DRC/Office=single devices
        site_handlers: dict[str, str] = {
            "Site_FGT-DC": "ha",
            "Site_FGT-DRC": "single",
            "Site_FGT_Office": "single",
        }
        sites_to_query = [s for s in site_list if s in site_handlers]
        # If no specific sites selected, use all 3
        if not sites_to_query:
            sites_to_query = list(site_handlers.keys())

        try:
            from app.opensearch import ha as ha_qb

            # ── DC: HA members (ha_member measurement) ──────────────
            if "Site_FGT-DC" in sites_to_query:
            ...
            if "Site_FGT-DRC" in sites_to_query:
                try:
            ...
            if "Site_FGT_Office" in sites_to_query:
                try:
    ...
    if report_type in ("R-07", "R-08") and (not sections or "executive_summary" in sections):
        ...
        try:
            ...
            if sla_summaries:
                ...
                met = sum(1 for s in sla_summaries if s.get("sla_compliance") == "Met")
            ...
            # ── Throughput Timeline ─────────────────────────────────────────
            timeline = to.get("throughput_timeline")
            if timeline:
                summary["throughput_timeline"] = timeline

            # ── 2. Per-Site Status Matrix ───────────────────────────────────
            inbound_data = context.get("report_data", {}).get("traffic_inbound", {})
            internal_data = context.get("report_data", {}).get("traffic_internal", {})
            ...
            for site in site_list:
                ...
                overview_match = next(
                    (s for s in overview_sites if s and s.get("site") == site_label),
                    None,
                )
                ...
                inbound_sites = inbound_data.get("per_site_summary", [])
                inbound_match = next(
                    (s for s in inbound_sites if s and s.get("site") == site_label),
                    None,
                )
                ...
                internal_sites = internal_data.get("per_site_summary", [])
                internal_match = next(
                    (s for s in internal_sites if s and s.get("site") == site_label),
                    None,
                )
                if internal_match and internal_match.get("total_bytes"):
                    site_status["internal"] = internal_match["total_bytes"]

                # VPN count per site (SSL=DC, IPsec=DRC, Office=None)
                if site_label == "DC":
                ...
                # SD-WAN status (up/partial/down per site)
                sdwan_links = summary.get("sdwan_health") or []
                site_links = [l for l in sdwan_links if l.get("site") == site_label]
                if len(site_links) == 0:
                    site_status["sdwan"] = None
                elif all(l.get("status", "").upper() == "UP" for l in site_links):
                    site_status["sdwan"] = "up"
                elif any(l.get("status", "").upper() == "DOWN" for l in site_links):
                    site_status["sdwan"] = "down"
                else:
                    site_status["sdwan"] = "partial"
