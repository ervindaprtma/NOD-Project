...
| `DeviceMiniCard` | Compact device card for DC firewall pair (FW-1 & FW-2 side by side) |
| `DeviceWideCard` | Full-width device card for DRC/Office single devices |
...
#### Row 1: KPI Summary Cards (5 cards, full width — compact layout)
...
| HA Status | Compact HA cluster health indicator | Red | `/dashboard/resources` |
| Alerts | Active alert count | Slate | `/dashboard/alerts` |

The HA Cluster detail card was removed from the KPI row. HA status is now shown as a **compact indicator** within the KPI cards rather than a full standalone card with detailed member information.

#### Row 2: Device Health + SD-WAN (mixed layout)

**DC site:** Two device cards displayed **side by side** using `DeviceMiniCard` — one for FW-1 and one for FW-2 (firewall pair). Each compact card shows:
- Hostname
- CPU / Memory utilization (MiniGauges)
- Session count

**DRC and Office sites:** Each uses a `DeviceWideCard` at **full width** with:
- Hostname, serial number
- CPU / Memory utilization (MiniGauges)
- Session count, RAM capacity

The SD-WAN link status cards are displayed alongside the device health section.

#### Row 3: WAN/MPLS Bandwidth (full width)

Full-width section with **5 interface cards** showing:
- Interface name
- In Mbps (blue background)
- Out Mbps (orange background)

**Dynamic interval:** The bandwidth time series uses a dynamic aggregation interval based on the selected time range to avoid excessive bucket counts:

| Time Range | Interval | Rationale |
|---|---|---|
| ≤ 2 hours | 60 seconds | Fine-grained for short windows |
| ≤ 12 hours | 5 minutes | Medium granularity |
| > 12 hours | 15 minutes | Coarse aggregation for large ranges |

Mbps calculation uses the actual `interval_seconds` value (not a hardcoded divisor) to ensure accurate throughput regardless of interval.

Sort order: WAN interfaces first (LinkNet, iForte, LDP), then MPLS interfaces (LinkNet, iForte).

#### Row 4: Top Client Internet (3-column, per site) — NEW

Three columns: **DC**, **DRC**, **Office**

Horizontal bar chart showing the top internet clients ranked by traffic volume. Each bar includes:
- Rank number
- Client IP address
- Colored progress bar
- Traffic value

Data sourced from the `top_clients` field in the traffic-flow summary API response.

#### Row 5: Top Application Internet Usage (3-column, per site)

Three columns: **DC**, **DRC**, **Office**

Horizontal bar charts showing top applications ranked by traffic volume (renamed from "Top Apps"). Each bar includes:
- Rank number
- Application name
- Colored progress bar
- Traffic value

#### Row 6: Top Destination Internet AS Org (3-column, per site)

Three columns: **DC**, **DRC**, **Office**

Horizontal bar charts using **emerald** color bars showing top destination AS organizations by traffic volume (renamed from "Top AS Org").

#### Row 7: Inbound VIP (2-column)

Two columns: **DC**, **DRC**

Horizontal bar charts using **violet** color bars showing inbound VIP traffic distribution.

#### Row 8: Top Customer AS — Inbound VIP (2-column)
...
| API | Calls | Purpose |
|---|---|---|
| Overview API | 1 | KPI metrics (VPN users, device count, alerts, HA status) |
| Traffic Flow Summary | 3 | Top clients + top apps + top AS org per site (DC, DRC, Office) |
| Resources API | 3 | Device health per site (DC, DRC, Office) |
| Traffic Inbound Summary | 2 | Inbound VIP + top customer AS (DC, DRC) |
...
The top_clients data is reused from the traffic-flow summary API (no separate client endpoint needed).
...
### 4.10 Reports Page (`app/dashboard/reports/page.tsx` — 728 lines)

**Route:** `/dashboard/reports`

Generates and manages **network reports** in PDF, HTML, and DOCX formats.

#### Report Types (8 types)

| ID | Title | Description |
|---|---|---|
| R-01 | Traffic Internet | Top apps, throughput, AS, countries, protocol, per-site breakdown |
...
|---|---|
| R-01 | Top Applications, Throughput Timeline, Top AS Orgs, Top Countries, Protocol Distribution, Per-Site Breakdown |
| R-02 | Device Status, CPU Timeline, Memory Timeline, Session Timeline |
| R-03 | SSL VPN, IPsec VPN |
| R-04 | Latency, Jitter, Packet Loss, Link Status |
| R-05 | Top Services, Top Client AS, Top Countries, Egress Interfaces |
| R-06 | Top Services, Top Client IPs, Top Server IPs |

#### Output Formats

| Format | Extension | Notes |
|---|---|---|
| PDF | `.pdf` | WeasyPrint rendering, A4 layout |
| HTML | `.html` | Interactive, chart images embedded |
| DOCX | `.docx` | python-docx, tables + embedded charts |
...
2. Select sites (multi-select: DC, DRC, Office)
3. Select output format (PDF, HTML, DOCX)
4. Select time range (preset or custom datetime-local)
5. Optionally select sections to include (checkboxes)
6. Click "Generate" button
7. POST to `/api/v1/reports/generate`
8. Returns 202 with `job_id`

#### Job List

Displays previously generated reports with:
- Report type and sites
- Output format badge
- Time range
- File size
- Status badges:
...
| Action | Method | Notes |
|---|---|---|
| Download | Fetch with auth headers → blob download | Works for all formats |
| Preview | `window.open(url)` | HTML only — opens in new tab |
| Distribute | POST to `/api/v1/reports/distribute/{job_id}` | Email, Telegram, Discord, WhatsApp |
...
- Preview only works for HTML format (no auth header sent for other formats)
